...
CREATE TABLE public.transactions (
  ...
  type TEXT NOT NULL CHECK (type IN ('Survey Reward', 'Offer Reward', 'Withdrawal Request', 'Withdrawal Approval', 'Withdrawal Rejection', 'Manual Credit', 'Chargeback', 'Referral Commission')),
...
CREATE POLICY "Users can insert withdrawal requests" ON public.transactions 
FOR INSERT WITH CHECK (
  auth.uid() = user_id AND 
  type = 'Withdrawal Request' AND 
  status = 'Pending'
);
...
CREATE POLICY "Users can update their own login history" ON public.login_history FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Admins can manage all login history" ON public.login_history FOR ALL USING (public.is_admin());

-- FUNCTION: Handle Auto-Create User Profile
...
DECLARE
  ...
  last_n TEXT;
BEGIN
  -- Try to extract first and last name from metadata (common in OAuth)
  first_n := COALESCE(
  ...
  last_n := COALESCE(
    ...
    substring(new.raw_user_meta_data->>'full_name' from position(' ' in COALESCE(new.raw_user_meta_data->>'full_name', '')) + 1),
    substring(new.raw_user_meta_data->>'name' from position(' ' in COALESCE(new.raw_user_meta_data->>'name', '')) + 1),
    ''
  );

  INSERT INTO public.users (id, email, first_name, last_name, status, is_admin)
  VALUES (
    new.id, 
    new.email, 
    first_n, 
...
-- FUNCTION: Handle Referral Commission
...
DECLARE
  ...
  friend_name TEXT;
BEGIN
  -- Only for positive earnings that just became 'Completed'
  -- Avoid infinite loops by checking type != 'Referral Commission'
  IF (NEW.status = 'Completed' AND NEW.amount > 0 AND NEW.type IN ('Survey Reward', 'Offer Reward', 'Manual Credit')) THEN
    -- Check if OLD.status wasn't already 'Completed' (to avoid double commission on updates)
    IF (TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND OLD.status != 'Completed')) THEN
      
      SELECT referred_by, full_name INTO referrer_id, friend_name FROM public.users WHERE id = NEW.user_id;
