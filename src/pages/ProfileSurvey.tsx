    ...
    switch (currentStep) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-5 py-4 focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all font-medium"
                />
              </div>
              <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-3">
                {['Male', 'Female', 'Other'].map(g => (
                  <button
                    ...
                    className={`py-4 rounded-2xl font-bold transition-all border ${formData.gender === g ? 'bg-primary text-white border-primary shadow-lg shadow-primary/20' : 'bg-gray-50 text-gray-600 border-gray-100 hover:bg-gray-100'}`}
                  >
                    {g}
                  </button>
                ))}
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <select 
                ...
                className="w-full bg-gray-50 border border-gray-100 rounded-2xl px-5 py-4 focus:bg-white focus:ring-2 focus:ring-primary/20 transition-all font-medium outline-none"
              >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-sm font-bold text-gray-700">State / Region</label>
                <input 
                  type="text" 
      ...
      case 2:
        return (
          <div className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-700">Employment Status</label>
              <select 
      ...
      case 3:
        return (
          <div className="space-y-6">
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-700">Annual Household Income</label>
              <select 
            ...
            <div className="space-y-2">
              <label className="text-sm font-bold text-gray-700">Number of Children</label>
              <select 
              ...
              <div className="flex flex-wrap gap-2">
                {['Smartphone', 'Laptop', 'Desktop', 'Tablet', 'Smart Watch', 'Gaming Console'].map(d => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${formData.devices.includes(d) ? 'bg-primary/10 border-primary text-primary' : 'bg-gray-50 border-gray-100 text-gray-500'}`}
                  >
                    {d}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {['Technology', 'Fashion', 'Gaming', 'Travel', 'Cooking', 'Fitness', 'Automotive', 'Investing', 'Movies', 'Music'].map(i => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${formData.interests.includes(i) ? 'bg-primary/10 border-primary text-primary' : 'bg-gray-50 border-gray-100 text-gray-500'}`}
                  >
                    {i}
                  </button>
                ))}
              ...
              <div className="flex flex-wrap gap-2">
                {['Online Shopping', 'In-Store', 'Groceries', 'Luxury Goods', 'Electronics', 'Home Decor'].map(s => (
                  <button
                    ...
                    className={`px-4 py-2 rounded-xl text-xs font-bold border transition-all ${formData.shoppingHabits.includes(s) ? 'bg-primary/10 border-primary text-primary' : 'bg-gray-50 border-gray-100 text-gray-500'}`}
                  >
                    {s}
                  </button>
                ))}
          ...
          <div className="space-y-6">
            <div className="bg-primary/5 rounded-[2rem] p-8 border border-primary/10">
              ...
              <div className="mt-8 pt-8 border-t border-primary/10 grid grid-cols-1 gap-6">
                 <div>
                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Interests & Devices</p>
                    <div className="flex flex-wrap gap-2">
                       {[...formData.devices, ...formData.interests].slice(0, 8).map(tag => (
                         <span key={tag} className="px-3 py-1 bg-white border border-primary/20 rounded-full text-[10px] font-bold text-primary">
                           {tag}
                         </span>
                       ))}
                    </div>
    ...
    <UserLayout>
      <div className="max-w-4xl mx-auto py-8">
        ...
        <div className="text-center mb-12">
          ...
          <h1 className="text-4xl font-black text-gray-900 mb-4 tracking-tight">Complete Profile Survey</h1>
        ...
        {/* Progress Tracker */}
        <div className="bg-white border border-gray-100 rounded-[2.5rem] p-6 mb-8 shadow-sm overflow-x-auto">
          <div className="flex items-center justify-between min-w-[600px] px-4">
            {STEPS.map((step, index) => (
              <React.Fragment key={step.id}>
                <div className="flex flex-col items-center gap-2 relative z-10">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-500 ${
                    index <= currentStep ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'bg-gray-50 text-gray-400 border border-gray-100'
                  }`}>
                ...
                {index < STEPS.length - 1 && (
                  <div className="flex-1 h-0.5 bg-gray-100 mx-4 relative overflow-hidden">
                    <motion.div 
                      initial={{ width: '0%' }}
        ...
        {/* Form Content */}
        <div className="bg-white border border-gray-100 rounded-[2.5rem] shadow-xl shadow-gray-200/20 overflow-hidden min-h-[500px] flex flex-col">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                <div className="mb-10">
                  ...
                  <p className="text-gray-500 font-medium text-sm mt-1">Step {currentStep + 1} of {STEPS.length}</p>
                </div>
          ...
          <div className="p-8 bg-gray-50 border-t border-gray-100 flex items-center justify-between">
            <button
              ...
              className={`flex items-center gap-2 px-6 py-4 rounded-2xl font-bold uppercase tracking-widest text-[10px] transition-all ${
                currentStep === 0 ? 'opacity-0 cursor-default' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-100'
              }`}
            >
            ...
            {currentStep === STEPS.length - 1 ? (
              ...
              <button
                onClick={handleNext}
                className="flex items-center gap-2 px-10 py-4 bg-gray-900 text-white rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-lg shadow-gray-900/10 hover:scale-105 active:scale-95 transition-all"
              >
