...
export default function UserDashboard() {
  ...
  }, [transactions, settings.providers, settings.paymentMethods]);
      ...
      <div className="space-y-10">
        {!currentUser?.profileSurveyCompleted && (
          <motion.div
            ...
            className="bg-gradient-to-r from-orange-500 to-primary rounded-[2rem] p-5 md:py-6 md:px-8 shadow-xl shadow-primary/20 relative overflow-hidden group"
          >
            {/* Decoration */}
            <div className="absolute top-0 right-0 w-64 h-full bg-white/10 -skew-x-12 translate-x-1/2 group-hover:translate-x-1/3 transition-transform duration-700" />
            <div className="absolute -bottom-8 -left-8 w-32 h-32 bg-black/5 rounded-full blur-2xl" />
            
            <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-5">
              <div className="flex items-center gap-5 text-center md:text-left flex-col md:flex-row">
                <div className="w-12 h-12 bg-white/20 backdrop-blur-md rounded-xl flex items-center justify-center border border-white/30 shadow-inner shrink-0">
              ...
              <Link 
                to="/profile-survey"
                className="px-7 py-3.5 bg-white text-primary rounded-xl font-black uppercase tracking-widest text-[10px] hover:bg-orange-50 transition-all shadow-lg hover:scale-105 active:scale-95 flex items-center gap-2 shrink-0"
              >
          ...
          {[
            ...
            return (
              <motion.div
                ...
                className="bg-white border border-gray-100 rounded-[1.5rem] p-6 hover:border-primary/30 transition-all shadow-sm hover:shadow-md relative overflow-hidden group"
              >
                <div className="flex items-center justify-between mb-4 relative z-10">
                  <p className="text-xs font-bold text-gray-500 uppercase tracking-widest">{stat.label}</p>
                  <div className="w-10 h-10 rounded-xl bg-orange-50/50 border border-orange-100 flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-white transition-colors">
                    <Icon className="w-4 h-4" />
                  </div>
                </div>
                <p className="text-3xl font-black text-gray-900 tracking-tight relative z-10">{stat.value}</p>
                {/* Decorative background accent */}
                <div className="absolute -bottom-6 -right-6 w-24 h-24 bg-gradient-to-br from-orange-50 to-transparent rounded-full opacity-0 group-hover:opacity-100 blur-xl transition-opacity"></div>
              </motion.div>
            );
          })}
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
          ...
          <div className="xl:col-span-2 space-y-10">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-black text-gray-900 tracking-tight font-display">Top Offer Providers</h2>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {offerwallProviders.slice(0, 3).map((provider: any, i: number) => {
                  const Icon = Target;
                  return (
                    <div key={provider.slug || i} className="bg-white border border-gray-100 rounded-[1.5rem] p-8 shadow-sm hover:shadow-xl hover:border-primary/40 hover:-translate-y-1 transition-all duration-300 group flex flex-col cursor-pointer">
                      <div className="flex items-start justify-between mb-6">
                        <div className={`w-14 h-14 rounded-2xl border ${'bg-gray-50 text-gray-500 border-gray-200'} flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform overflow-hidden`}>
                          {provider.logoUrl ? (
                        ...
                        {provider.featured ? (
                          ...
                          <span className="px-2.5 py-1 bg-green-50 border border-green-100 text-green-700 rounded-md text-[10px] font-bold uppercase tracking-widest">
                            Active
                          </span>
                        )}
                      </div>
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-2xl font-black text-gray-900 tracking-tight font-display">Top Survey Providers</h2>
                ...
                {surveyProviders.slice(0, 3).map((provider: any, i: number) => {
                  ...
                  return (
                    <div key={provider.slug || i} className="bg-white border border-gray-100 rounded-[1.5rem] p-8 shadow-sm hover:shadow-xl hover:border-primary/40 hover:-translate-y-1 transition-all duration-300 group flex flex-col cursor-pointer">
                      ...
                      <h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-primary transition-colors">{provider.name}</h3>
                      <p className="text-sm font-medium text-gray-500 mb-8 flex-1 leading-relaxed">{provider.description}</p>
                      <Link to={`/surveys/${provider.slug}`} className="w-full py-3.5 bg-gray-50 group-hover:bg-primary text-gray-600 group-hover:text-white rounded-xl text-xs font-bold uppercase tracking-widest transition-colors flex items-center justify-center gap-2 border border-gray-100 group-hover:border-primary shadow-sm">
                        Open <ChevronRight className="w-3 h-3 group-hover:translate-x-0.5 transition-transform" />
                      </Link>
                    </div>
                  );
          ...
          <div className="xl:col-span-1">
            <motion.div
              ...
              className="bg-white border border-gray-100 rounded-[2rem] p-8 shadow-sm hover:shadow-md transition-shadow relative"
            >
              <div className="flex items-center justify-between mb-8 relative z-10">
                <div className="flex items-center gap-3">
                  <div className="w-2 h-2 rounded-full bg-green-400 shadow-[0_0_8px_rgba(74,222,128,0.6)] animate-pulse"></div>
                  <h2 className="text-xl font-black text-gray-900 tracking-tight font-display">Recent Activity</h2>
                </div>
              </div>
              ...
              <div className="relative z-10">
                <div className="space-y-0 relative">
                  {recentActivities.length > 0 ? recentActivities.map((activity, i) => {
                    return (
                      <div key={i} className={`relative flex items-center gap-4 py-3 group ${i !== recentActivities.length - 1 ? 'border-b border-gray-100' : ''}`}>
                        ...
                        <div className="relative shrink-0">
                          ...
                          />
                          <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-md flex items-center justify-center border-2 border-white shadow-sm overflow-hidden ${activity.providerLogo ? 'bg-white' : activity.fallbackColor}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between mb-0.5">
                            <p className="text-sm font-bold text-gray-900 truncate pr-2">{activity.type}</p>
                            <p className={`text-sm font-black shrink-0 ${activity.type.includes('Withdrawal') ? 'text-gray-900' : 'text-green-600'}`}>{activity.amount}</p>
                          </div>
                          <div className="flex items-center justify-between">
                            <p className="text-xs font-semibold text-gray-500 truncate">
                              {activity.providerName}
                            </p>
                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider shrink-0 flex items-center">
                              {activity.time}
                              {activity.status === 'Pending' && (
                                <span className="text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded border border-amber-100 ml-2">PENDING</span>
                              )}
                            </p>
                          </div>
                        </div>
                    ...
                    <div className="py-8 text-center text-gray-400 text-sm font-medium">No recent activity yet.</div>
                  )}
                </div>
              </div>
              
              <button className="relative z-10 w-full mt-8 py-3.5 border border-gray-200 text-gray-600 hover:text-gray-900 rounded-xl text-xs font-bold hover:bg-gray-50 transition-colors uppercase tracking-widest font-medium">
                View Full Feed
              </button>
            </motion.div>
          </div>
