...
function FAQItem({ question, answer, isOpen, onClick }: FAQItemProps) {
  return (
    <div className="border border-theme-border rounded-3xl bg-theme-surface overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300">
      <button
        onClick={onClick}
        className="w-full flex items-center justify-between p-6 text-left focus:outline-none"
      >
        <span className="text-theme-text font-bold text-base md:text-lg pr-8">
          {question}
        </span>
        <div className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${isOpen ? 'bg-primary text-white rotate-180' : 'bg-theme-bg text-theme-text-muted opacity-60'}`}>
      ...
      <AnimatePresence initial={false}>
        {isOpen && (
          ...
          >
            <div className="px-6 pb-6 text-theme-text-muted font-medium leading-relaxed border-t border-gray-50 pt-4">
              {answer}
            </div>
          </motion.div>
        )}
...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="py-24 bg-theme-surface relative overflow-hidden">
        {/* Background elements */}
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center mb-16">
          <motion.h2
            ...
            className="text-3xl md:text-5xl font-extrabold text-theme-text font-display tracking-tight mb-4"
          >
        ...
        <motion.div 
            ...
            className="mt-16 p-8 rounded-[2.5rem] bg-theme-bg border border-theme-border text-center"
        >
            <p className="text-theme-text font-bold mb-4">Still have questions?</p>
