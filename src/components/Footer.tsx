...
export default function Footer() {
  ...
  return (
    <footer id="contact" className="bg-theme-text pt-20 pb-10 overflow-hidden relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8 mb-16">
          ...
          <div className="lg:col-span-4">
            ...
            <p className="text-theme-text-muted opacity-60 font-medium leading-relaxed mb-8 max-w-sm">
              {settings.tagline}
            </p>
            <div className="flex gap-4">
              {socialLinks.map((social, idx) => (
                <a
                  key={idx}
                  href={social.href}
                  className="w-10 h-10 rounded-full bg-gray-800 flex items-center justify-center text-theme-text-muted opacity-60 hover:bg-primary hover:text-white transition-all duration-300 overflow-hidden"
                >
                  {social.logoUrl ? (
              ...
              {quickLinks.map((link) => (
                <li key={link.id}>
                  {link.id === 'contact' ? (
                    <Link
                      to="/contact"
                      className="text-theme-text-muted opacity-60 hover:text-primary transition-colors font-medium text-sm"
                    >
                      {link.name}
                    </Link>
                  ) : (
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-theme-text-muted opacity-60 text-xs font-medium mb-4">Get the latest survey opportunities delivered to your inbox.</p>
        ...
        <div className="pt-8 border-t border-gray-800 flex flex-col md:flex-row justify-between items-center gap-4">
          ...
          <div className="flex items-center gap-6">
             <span className="text-theme-text-muted text-xs font-bold uppercase tracking-widest">Global Rewards System</span>
